...
import mergeStateAndComputed from './utils/mergeStateAndComputed';
...
export default function ({
  ...
  this.mergeReducers = (nextReducers: T.ModelReducers = {}) => {
    ...
    return combineReducers(this.reducers, computeds);
  };
  ...
  this.createModelReducer = (model: T.Model) => {
    ...
    const combinedReducer = (
      state: any = mergeStateAndComputed(model, model.state),
      action: T.Action,
    ) => {
      if (typeof modelReducers[action.type] === 'function') {
        const context = {};
        Object.keys(modelReducers).forEach(name => {
          if (name.indexOf(`${model.name}/`) === 0) {
            context[name.replace(`${model.name}/`, '')] = payload =>
              modelReducers[name].call(context, state, payload);
          }
        });
        // 允许同一个 model 中的所有 reducer 通过 this 互相调用
        return modelReducers[action.type].call(
          context,
          state,
          action.payload,
          action.meta,
  ...
  //TODO  目前是直接合并 computed 到 model state
  // const oldGetState = this.store.getState;
  // // this.store.getState = () => {
  // //   console.log('run getState');

  // //   const state = oldGetState();
  // //   // for (const key in state) {
  // //   //   state[key] = mergeStateAndComputed(config.models[key], state[key]);
  // //   // }
